 ...
 * <p>
 * https://github.com/ZhouHuang23/EasyAdapter
 */
public abstract class EasyAdapter<VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH> implements View.OnClickListener {
    ...
    public void onBindViewHolder(@NonNull VH holder, int position) {
        ...
        holder.itemView.setTag(position);// itemView 绑定位置
        holder.itemView.setOnClickListener(this);

        if (selectMode == SelectMode.CLICK) { //点击
            holder.itemView.setSelected(false);
        } else if (selectMode == SelectMode.SINGLE_SELECT) { //单选
            holder.itemView.setSelected(singleSelected == position);
        } else if (selectMode == SelectMode.MULTI_SELECT) {//多选
            holder.itemView.setSelected(multiSelected.contains(position));
        }
    }
    ...
    public boolean isSelectSingleMode() {
        return (selectMode == SelectMode.SINGLE_SELECT);
    }
    ...
    public boolean isSelectMultiMode() {
        return (selectMode == SelectMode.MULTI_SELECT);
    }
